...
SelectTweetsAndWriteToCSV <- function(list){
  ...
  for (keyword in list){
    query = paste("select tweet_text, tweet_datetime, latitude, longitude from tweets2601 where tweet_text like '%", keyword, "%'", sep = "")
...
# Get the GADM boundaries before the for loop
